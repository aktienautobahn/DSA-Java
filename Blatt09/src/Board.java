...
public class Board {
    ...
    public int manhattan() {
        // TODO 1.1 manhattan
        int sizeOfBoard = nx * ny;
        int rowIndex, columnIndex, token;
        int targetRowIndex, targetColumnIndex;
        int distance = 0;
        for (int i = 0; i < sizeOfBoard; i++) { // i = mustToken
            rowIndex = i / (nx); // row index is i / nx
            columnIndex = i % (nx); // col index is i mod nx
            token = field[rowIndex][columnIndex]; /// token = isToken

            if (token == 0) continue; // Skipping the blank tiles

            targetRowIndex = (token - 1) / nx; // target row
            targetColumnIndex = (token - 1) % nx; // target column

                distance += Math.abs(targetRowIndex - rowIndex) + Math.abs(targetColumnIndex - columnIndex);
